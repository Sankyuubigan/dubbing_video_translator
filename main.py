...
class App:
    ...
    def actual_processing(self, video_file_path, local_srt_path, op_key,
        ...
        try:
            ...
            if srt_to_use_for_transcription:
                if not os.path.exists(srt_to_use_for_transcription):
                    ...
                    if step_name_parse in dub_steps:
                        ...
                        # Вызываем parse_srt_file с post_process=True
                        parsed_srt_segments = transcriber.parse_srt_file(srt_to_use_for_transcription, post_process=True)
                        if not parsed_srt_segments:
                            self._update_status(f"⚠️ SRT parsing (and post-processing) failed for {os.path.basename(srt_to_use_for_transcription)}. Will attempt WhisperX transcription.", append=True)
                            srt_to_use_for_transcription = None
                        else:
                            self.log_time(step_name_parse, start_t)
                            # Остальная логика определения языка и пропуска перевода остается
                            ...
                            if step_name_diar_srt in dub_steps:
                                ...
                                if diarization_data_for_cloning is not None and not diarization_data_for_cloning.empty:
                                    ...
                                    aligned_segments = parsed_srt_segments # parsed_srt_segments уже содержат 'SPEAKER_00' по умолчанию
            ...
            # Очистка текста от HTML-тегов (если еще не сделано)
            ...
            translated_segments = list(aligned_segments) # Копируем для перевода
            ...
            if step_name_trans in dub_steps:
                if not skip_translation_step:
                    ...
                    translator.load_translator_model(device=device); translated_segments = translator.translate_segments(aligned_segments) # Переводим из aligned_segments
                    ...
                    for seg_idx, seg_val in enumerate(translated_segments): # Обновляем translated_text в translated_segments
            ...
            if step_name_va in dub_steps:
                ...
                if os.path.exists(output_path):
                    self._update_status(f"INFO: Output file {os.path.basename(output_path)} already exists. Attempting to delete it before creating a new one.", append=True)
                    try:
                        os.remove(output_path)
                        self._update_status(f"INFO: Successfully deleted existing file: {os.path.basename(output_path)}", append=True)
                    except OSError as e_del:
                        self._update_status(f"WARNING: Could not delete existing output file {os.path.basename(output_path)}: {e_del}. FFmpeg will attempt to overwrite.", append=True)
            ...
            if output_path and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                if output_path: self.root.after(0, lambda: messagebox.showwarning("Warning", f"Operation '{op_key}' finished, but the output file seems to be missing or empty."))
